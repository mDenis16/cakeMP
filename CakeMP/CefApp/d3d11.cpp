...
namespace d3d11 {
		...
		, ctx_(make_shared<Context>(pctx))
	{
		...
		_lib_compiler = LoadLibrary("d3dcompiler_47.dll");
	}
	...
	unique_ptr<SwapChain> Device::create_swapchain(HWND window, int width, int height)
	{
		HRESULT hr;
		...
		return  std::make_unique<SwapChain>(swapchain, rtv, sampler, blender);
	}
		...
		string const& model)
	{
		...
		if (!fnc_compile) {
			sbp::logWrite("ERROR!! Cant get GetProcAddress(D3DCompile)");
			return nullptr;
		}
	...
	//
	unique_ptr<Effect> Device::create_default_effect()
	{
		sbp::logWrite("Compiling default effect");
		auto const vsh =
			R"--(struct VS_INPUT
{
	float4 pos : POSITION;
...
float4 main(VS_OUTPUT input) : SV_Target
{
	...
	unique_ptr<Effect> Device::create_effect(
		...
		string const& pixel_model)
	{
		...
		return make_unique<Effect>(vshdr, pshdr, layout);
	}
